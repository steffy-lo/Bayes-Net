...
def q1(bn):
    # p(sq = good | pv = strong) =
    # p(sq = good | pv = strong, bv = strong) =
    # p(sq = good | pv = strong, bv = weak) =
    # p(sq = good | pv = strong, bv = dead)
    ...
    n2 = VE(bn, sq, [pv, mf])
    pv.set_evidence('strong')
    bv.set_evidence('dead')
    n3 = VE(bn, sq, [pv, mf])
    print(n0[0], '=', n1[0], '=', n2[0], '=', n3[0])

def q2(bn):
    # increase
    # p(ds = okay) < p(ds = okay | pv = strong)
    # p(mf = okay) < p(mf = okay | pv = strong)
    # p(bv = strong) < p(bv = strong | pv = strong)
    b1 = VE(bn, ds, [])
    b2 = VE(bn, mf, [])
    b3 = VE(bn, bv, [])
    pv.set_evidence('strong')
    ...
    # decrease
    # p(ds = okay | pv = strong) >
    # p(ds = okay | pv = strong, mf = okay)
    # p(bv = strong | pv = strong) >
    # p(bv = strong | pv = strong, mf = okay)
    pv.set_evidence('strong')
    mf.set_evidence('okay')
    n1 = VE(bn, ds, [pv, mf])
    ds.set_evidence('okay')
    n3 = VE(bn, bv, [pv, mf])
    print(n1[0], '<', a1[0])
    print(n3[0], '<', a3[0])
...
def q3(bn):
    # p(pv = okay) <
    # p(pv = okay | ds = okay) <
    # p(pv = okay | ds = okay, mf = okay) <
    # p(pv = okay | ds = okay, mf = okay, bv = strong)
    n0 = VE(bn, pv, [])
    ds.set_evidence('okay')
    n1 = VE(bn, pv, [ds])
    mf.set_evidence('okay')
...
def q4(bn):
    ...
    # p(pv = okay | ds = okay) <
    # p(pv = okay | ds = okay, mf = okay) >
    # p(pv = okay | ds = okay, mf = okay, bv = weak)
    n0 = VE(bn, pv, [])
    ds.set_evidence('okay')
    n1 = VE(bn, pv, [ds])
    mf.set_evidence('okay')
